...
let handler = async (message, { conn, text }) => {
  ...
  try {
    await message.react('⏳') 
    conn.reply(message.chat, '『 ⇓⇓ 』 Buscando videos en TikTok, espere un momento...', message)

    // ✅ Nueva API
    let { data: response } = await axios.get(`https://api.tiklydown.eu.org/api/search?query=${encodeURIComponent(text)}`)

    if (!response.data || response.data.length === 0) {
      return conn.reply(message.chat, '『 ⚠️ 』 No se encontraron resultados.', message)
    }

    let searchResults = response.data.slice(0, 5) // Limita a 5 resultados

    let results = await Promise.all(searchResults.map(async result => {
      let videoMessage = await generateWAMessageContent({ video: { url: result.video_url } }, { upload: conn.waUploadToServer })
      return {
