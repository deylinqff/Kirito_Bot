...
let handler = async (m, { conn, usedPrefix, command, text }) => {
  ...
  if (isQuotedImage) {
    ...
    try {
      ...
      // **Aquí se agregan los botones**
      const buttons = [
        { buttonId: `${usedPrefix}help`, buttonText: { displayText: 'Ayuda' }, type: 1 },
        { buttonId: `${usedPrefix}menu`, buttonText: { displayText: 'Menú Principal' }, type: 1 },
        { buttonId: `${usedPrefix}credits`, buttonText: { displayText: 'Créditos' }, type: 1 },
      ];

      const buttonMessage = {
        text: response,
        footer: '🤖 Kirito-Bot powered by Deylin',
        buttons: buttons,
        headerType: 1,
      };
